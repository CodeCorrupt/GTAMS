...
* Added `command.request_options` to `Guzzle\Service\Command\AbstractCommand` to pass request options to requests
      created by a command (e.g. custom headers, query string variables, timeout sessions, etc).
...
* Added the ability to overwrite configuration sessions in a client when grabbing a throwaway client from a builder.
...
* Services defined in two different config files that include one another will by default replace the previously
  defined service, but you can now create services that extend themselves and merge their sessions over the previous
